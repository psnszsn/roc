...
pub const LiteralDispatchPlan = extern struct {
    ...
    pub fn dispatchResolution(self: LiteralDispatchPlan) Resolution {
        return @fromBackingInt(@intCast(self.resolution));
    }
};
...
comptime {
    ...
    const diagnostic_field_names = @typeInfo(CIR.Diagnostic).@"union".field_names;
    std.debug.assert(diagnostic_field_names.len == MODULEENV_DIAGNOSTIC_NODE_COUNT);
}

comptime {
    // Check the number of CIR.Expr nodes
    const expr_field_names = @typeInfo(CIR.Expr).@"union".field_names;
    std.debug.assert(expr_field_names.len == MODULEENV_EXPR_NODE_COUNT);
}

comptime {
    // Check the number of CIR.Statement nodes
    const statement_field_names = @typeInfo(CIR.Statement).@"union".field_names;
    std.debug.assert(statement_field_names.len == MODULEENV_STATEMENT_NODE_COUNT);
}

comptime {
    // Check the number of CIR.TypeAnno nodes
    const type_anno_field_names = @typeInfo(CIR.TypeAnno).@"union".field_names;
    std.debug.assert(type_anno_field_names.len == MODULEENV_TYPE_ANNO_NODE_COUNT);
}

comptime {
    // Check the number of CIR.Pattern nodes
    const pattern_field_names = @typeInfo(CIR.Pattern).@"union".field_names;
    std.debug.assert(pattern_field_names.len == MODULEENV_PATTERN_NODE_COUNT);
}
...
pub fn recordLiteralDispatchPlan(
    ...
    var plan = LiteralDispatchPlan{
        ...
        .resolution = @backingInt(LiteralDispatchPlan.Resolution.unresolved),
    };
    const plan_plus_one = literalDispatchPlanPlusOne(node);
    if (plan_plus_one != 0) {
        plan.resolution = store.literal_dispatch_plans.get(@fromBackingInt(@intCast(plan_plus_one - 1))).resolution;
...
pub fn finalizeLiteralDispatchResolution(
    ...
    plan.resolution = @backingInt(resolution);
    store.literal_dispatch_plans.set(@fromBackingInt(@intCast(plan_plus_one - 1)), plan);
}
...
pub fn replaceExprWithTypeDispatchCall(
    ...
    node.setPayload(.{ .expr_type_dispatch_call = .{
        .type_dispatch_stmt = @backingInt(type_dispatch_stmt),
        .method_name = @bitCast(method_name),
        .method_call_data_idx = method_call_data_idx,
        .constraint_fn_var = @backingInt(constraint_fn_var),
    } });
...
pub fn replaceExprWithRuntimeError(
    ...
    node.setPayload(.{ .diag_single_value = .{
        .value = @backingInt(diagnostic_idx),
    } });
    store.nodes.set(node_idx, node);
}
...
pub fn getWhereClause(store: *const NodeStore, whereClause: CIR.WhereClause.Idx) CIR.WhereClause {
    ...
    switch (tag) {
        .where_method, .where_method_effectful => {
            ...
            const var_ = @as(CIR.TypeAnno.Idx, @fromBackingInt(@intCast(p.var_idx)));
            ...
            return CIR.WhereClause{ .w_method = .{
                ...
                .ret = @fromBackingInt(@intCast(args_ret.node)),
    ...
    switch (whereClause) {
        .w_method => |where_method| {
            ...
            _ = try store.span_with_node_data.append(store.gpa, .{
                ...
                .node = @backingInt(where_method.ret),
            });
...
test "NodeStore basic CompactWriter roundtrip" {
    ...
    try original.recordLiteralDispatchPlan(node1_idx, .numeral, @fromBackingInt(@intCast(7)), @fromBackingInt(@intCast(9)));
