...
test "boxy abi structural equality compares scalars through a descriptor" {
    ...
    var b: u64 = 42;
    var c: u64 = 7;
    try std.testing.expect(boxy_abi.roc_boxy_eq(@ptrCast(&a), @ptrCast(&b), @backingInt(layout_mod.Idx.u64), &descs[0]));
    try std.testing.expect(!boxy_abi.roc_boxy_eq(@ptrCast(&a), @ptrCast(&c), @backingInt(layout_mod.Idx.u64), &descs[0]));
}
...
test "boxy abi reentrant inspect specialization keeps descriptors outside per-call scratch" {
    ...
    const descs = [_]BoxyTypeDesc{
        .{ .payload_layout = .u64, .contains_refcounted = false, .inspect_method = @fromBackingInt(@intCast(fixtureTableIndex(0))) },
        .{ .payload_layout = .u64, .contains_refcounted = false, .inspect_method = @fromBackingInt(@intCast(fixtureTableIndex(0))) },
        .{ .payload_layout = .u64, .contains_refcounted = false },
        .{ .payload_layout = .u64, .contains_refcounted = false },
        .{ .payload_layout = .u64, .contains_refcounted = false },
...
test "boxy builtin parameter ABI sizes match the wrapper declarations" {
    ...
    inline for (@typeInfo(BoxyBuiltinFn).@"enum".field_values) |field_value| {
        ...
        const param_types = @typeInfo(@TypeOf(@field(eval.boxy_abi, boxy_fn.symbolName()))).@"fn".param_types;

        if (boxy_fn.paramAbiSizes()) |sizes| {
            try std.testing.expectEqual(param_types.len, sizes.len);
            inline for (param_types, 0..) |param_type, i| {
                errdefer std.debug.print(
                    "{s} parameter {d} is {s}\n",
                    .{ boxy_fn.symbolName(), i, @typeName(param_type.?) },
                );
                try std.testing.expectEqual(@as(u8, @sizeOf(param_type.?)), sizes[i]);
            }
        } else {
            errdefer std.debug.print(
                "{s} takes {d} parameters and needs a paramAbiSizes row\n",
                .{ boxy_fn.symbolName(), param_types.len },
            );
            try std.testing.expect(param_types.len <= max_int_param_regs);
        }
    }
}
