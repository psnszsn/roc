...
fn writeBuiltinIndicesZig(
    ...
    const builtin_indices_info = @typeInfo(BuiltinIndices).@"struct";
    inline for (builtin_indices_info.field_names, builtin_indices_info.field_types) |field_name, field_type| {
        const value = @field(indices, field_name);
        if (field_type == CIR.Statement.Idx) {
            try out.print("    .{s} = {d},\n", .{ field_name, @backingInt(value) });
        } else if (field_type == base.Ident.Idx) {
            try out.print("    .{s} = {d},\n", .{ field_name, @as(u32, @bitCast(value)) });
        } else {
            @compileError("unsupported BuiltinIndices field type: " ++ @typeName(field_type));
        }
    }
